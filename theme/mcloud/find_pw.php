...
<body class="bf-login">
    ...
    <div class="container mt-5">
        ...
        <div id="timer_auth" class="position-relative mt-4 mb-5" style="">
            ...
            <input type="text" id='hp_auth' class="bd-radius-none border-bottom-0" placeholder="인증번호 입력">
        </div>

        <div id="pw_form" style="">
            <input type="password" id='auth_pw' class="bd-radius-none border" placeholder='비밀번호 재설정' data-i18n="[placeholder]find_pw.비밀번호 재설정">
            <input type="password" id='re_auth_pw' class="bd-radius-none border" placeholder='비밀번호 재설정 확인' data-i18n="[placeholder]find_pw.비밀번호를 재설정 확인">
            <input type="button" class="btn btn_wd btn-agree btn--blue bd-radius-none" id='confirm_pw' value="확인" data-i18n="[value]find_pw.확인">
        </div>
...
<script>
    ...
    $('#confirm_pw').click(function() {
        ...
        if ((auth_pw.length < 4 || auth_pw.length > 8 || !pattern.test(auth_pw))) {
            $('#notice_password').show();
