...
/*레벨*/
if($mb_level != 10){
	$mb_level = member_level_proc($_POST['mb_deposit_point']);
}
...
$sql_common = "  mb_name = '{$mb_name}',
				 ...
				 mb_8 = '{$_POST['mb_8']}',
				 mb_9 = '{$_POST['mb_9']}',
				 mb_deposit_point = '{$_POST['mb_deposit_point']}',
				 mb_eth_account = '{$_POST['mb_eth_account']}',
				 mb_usdt_account = '{$_POST['mb_usdt_account']}',
				 center_use = '{$_POST['center_use']}',
...
else if ($w == 'u')
{
	...
	*/
	...
	// 업스테어 주문기록 
	if($upstair > 0){
		// $upstair_sql = "insert g5_shop_order set od_id = '{$od_id}', mb_no = '{$mb_no}', mb_id = '{$mb_id}',  od_cash = '{$upstair}', upstair = '{$upstair}', pv = '{$upstair}', od_cart_price = '{$upstair}', od_time = '{$today}',od_date = '{$todate}'od_settle_case = 'usdt',od_memo = 'admin' ";

		$upstair_sql = "insert g5_shop_order set
				od_id				= '{$od_id}'
				, mb_no             = '{$mb_no}'
				, mb_id             = '{$mb_id}'
				, od_cart_price     =  {$upstair}
				, od_cash    		= '1'
